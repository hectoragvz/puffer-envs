...
#define METEOR_JUMP_PENALTY 0.00f
...
#define DINO_POLICY_WEIGHTS_SHA256 \
    "abd2cc72e64bb4b72b623489307e345ca95e0004bfca71a9c79bf7bca9abba49"
...
void add_log(Dino* env) {
    ...
    env->log.reached_2x += env->episode_reached_2x;
    env->log.n++;
}
...
DinoSpeedChangeResult dino_change_speed(Dino* env, float new_speed) {
    ...
    if (new_speed == DINO_MAX_SPEED) env->episode_reached_2x = 1;
...
void c_step(Dino* env) {
    ...
    if (env->obstacle.x + env->obstacle.width <
        ...
        env->episode_return += env->rewards[0];
        spawn_obstacle(env);
        if (env->training_speedup_after_passes > 0 &&
                env->obstacles_passed == env->training_speedup_after_passes) {
